...
class TestRecurrenceTrackTemplate(CalendarMailCommon):
    """ Occurrences of a recurrence are generated by copying the base event.
    Modules hooking '_track_template' on 'calendar.event' (e.g. 'appointment')
    must not end up posting their template once per occurrence. """

    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.attendee = new_test_user(cls.env, 'trackee', email='trackee@example.com')
        cls.template = cls.env.ref('calendar.calendar_template_meeting_update')
    ...
    def setUp(self):
        ...
        def _track_template(self, changes):
            """ Mimics 'appointment': post a template based on tracked fields. """
            res = {}
            if 'location' in changes:
                res['location'] = (template, {'email_layout_xmlid': 'mail.mail_notification_light'})
            if 'active' in changes and not self.active:
                res['active'] = (template, {'email_layout_xmlid': 'mail.mail_notification_light'})
            return res

        self.patch(self.registry['calendar.event'], '_track_template', _track_template)

    def _tracked_messages(self, events):
        self.flush_tracking()
        return self.env['mail.message'].search([
            ('model', '=', 'calendar.event'),
            ('res_id', 'in', events.ids),
            ('subject', '=', self.template._render_field('subject', events.ids[:1])[events.ids[0]]),
        ])
    ...
    def _create_recurrence(self):
        event = self.env['calendar.event'].create({
            ...
            'stop': datetime(2019, 10, 22, 9, 0),
            'partner_ids': [(4, self.attendee.partner_id.id)],
            'recurrency': True,
            'rrule_type': 'weekly',
            'tue': True,
            'interval': 1,
        ...
        return event.recurrence_id or self.env['calendar.recurrence'].search(
            [('base_event_id', '=', event.id)])

    def test_recurrence_creation_tracks_once(self):
        """ Generating the occurrences must not repeat the tracking template. """
        recurrence = self._create_recurrence()
        events = recurrence.calendar_event_ids
        self.assertEqual(len(events), 5, "The recurrence should hold 5 occurrences")
        self.assertEqual(
            len(self._tracked_messages(events)), 1,
            "The tracking template must be posted once, not once per occurrence")

    def test_recurrence_archive_all_tracks_once(self):
        """ Cancelling a whole recurrence must notify the attendees exactly once. """
        recurrence = self._create_recurrence()
        events = recurrence.calendar_event_ids
        base_event = recurrence.base_event_id
        self.flush_tracking()
        before = len(self._tracked_messages(events))

        base_event.action_mass_archive('all_events')

        self.assertFalse(any(events.mapped('active')), "All occurrences should be archived")
        self.assertEqual(
            len(self._tracked_messages(events)) - before, 1,
            "Archiving a recurrence must post the tracking template once, not once per occurrence")

    def test_recurrence_archive_future_tracks_once(self):
        """ Same for 'this and following events'. """
        recurrence = self._create_recurrence()
        events = recurrence.calendar_event_ids.sorted('start')
        self.flush_tracking()
        before = len(self._tracked_messages(events))

        events[1].action_mass_archive('future_events')

        self.assertEqual(
            len(self._tracked_messages(events)) - before, 1,
            "Archiving future events must post the tracking template once")

    def test_recurrence_write_all_events_does_not_track(self):
        """ Updating every occurrence in batch must stay silent: the recurrence is
        rewritten, the meeting is not cancelled nor booked again. """
        recurrence = self._create_recurrence()
        events = recurrence.calendar_event_ids
        self.flush_tracking()
        before = len(self._tracked_messages(events))

        recurrence.base_event_id.write({
            'location': "Louvain-la-Neuve",
            'recurrence_update': 'all_events',
        })

        new_events = self.env['calendar.event'].with_context(active_test=False).search(
            [('id', 'in', events.ids)]) | recurrence.calendar_event_ids
        self.assertEqual(
            len(self._tracked_messages(new_events)) - before, 0,
            "Rewriting a recurrence must not post the tracking template per occurrence")
